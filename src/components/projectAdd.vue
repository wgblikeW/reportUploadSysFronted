    ...
    <el-dialog title="创建新的实验项目" :visible.sync="newProjectVisible" width="510px">
      <el-form :model="newProject" :rules="rules" ref="newProject" label-width="110px" class="demo-ruleForm"
        ...
        <el-form-item label="发布范围" prop="project_class" required>
          ...
          <el-button type="text" @click="newClassVisible = true" style="width: 60px;">新建班级</el-button>
        </el-form-item>
        <el-form-item label="截止时间" required>
          <el-col :span="12">
            <el-form-item prop="date1">
              <el-date-picker type="date" value-format="yyyy-MM-dd" placeholder="选择日期" v-model="newProject.date1"
                              style="width: 140px;"></el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item prop="date2">
              <el-time-select v-model="newProject.date2" :picker-options="{start: '00:00', step: '02:00', end: '23:00'}"
                              placeholder="选择时间" style="width: 140px;">
              </el-time-select>
            </el-form-item>
          </el-col>
        </el-form-item>
        <el-form-item label="实验项目文档">
          <el-upload class="upload-demo" action="http://localhost:8081/demo_war/addProject"
                     :before-upload="beforeUpload"
                     :data="newProject" :auto-upload="false" limit="1" accept=".PDF, .doc, .docx, .rar, .zip, .7z">
            <el-button size="small" type="text" @click="proj_nameload(scope.row.proj_name)">
              选择文件（仅能上传pdf/doc/docx/rar/zip/7z文件，且不超过10Mb）
            </el-button>
          </el-upload>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  methods: {
    submitProject () {
      var temp = JSON.parse(JSON.stringify(this.newProject))
      temp.project_class = require('js-base64').encode(temp.project_class)
      temp.project_course = require('js-base64').encode(temp.project_course)
      delete temp['class']
      this.$http.post('http://localhost:8081/demo_war/addProject', {
        newProject: temp
      }).then(res => {
        console.log(res)
      }).catch(err => {
